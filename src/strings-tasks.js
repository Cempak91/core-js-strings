...
function removeFirstOccurrences(str, value) {
  if (str.includes(value) === false) return str;
  return str
    .slice(0, str.indexOf(value))
    .concat(str.slice(str.indexOf(value) + value.length, 999999));
}
...
function removeLastOccurrences(str, value) {
  ...
  return str
    ...
    .concat(str.slice(str.lastIndexOf(value) + value.length, 999999));
}
...
function sumOfCodes(str) {
  ...
  let result = 0;
  for (let i = 0; i < String(str).length; i += 1) result += str.charCodeAt(i);
  return result;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (arr.includes(arrStr[i])) result += 1;
  }
  return result;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const elem = arr[i].split('').reverse();
    arr2 = `${arr2} ${elem.join('')}`;
  }
  return arr2.trim();
}
